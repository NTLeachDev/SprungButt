...
import com.nleachdev.derivativedi.framework.scanner.*;
...
public enum Containerr {
    INSTANCE;

    private static final Logger logger = LoggerFactory.getLogger(Containerr.class);
    ...
    private static final Map<BeanType, Set<Metadataa<?>>> metadataPerBeanType = new HashMap<>();
    private static final Map<String, Metadataa<?>> metadataPerBeanName = new HashMap<>();
    private static final Map<Class<?>, Set<String>> beanNamesPerType = new HashMap<>();

    public static Containerr getInstance() {
        return INSTANCE;
    }
